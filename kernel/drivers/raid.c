...
struct StorageDevice raids[RAID_MAX_CONTEXT];
...
int raid_init(void)
{
	...
	memset(raids, 0, sizeof(struct StorageDevice) * RAID_MAX_CONTEXT);
	return 0;
}
		...
		int driver_count, int stride)
{
	...
	for(x = 0;x < driver_count;x++)
	{
		...
		if(drivers[x]->sectors < sectors || sectors == -1)
			sectors = drivers[x]->sectors;
		if(sect_size == -1)
...
{
	struct StorageDevice* raid = driver->driver;
	...
	struct StorageDevice* disk = context->disks[disk_num];
...
int raid0_cache_dereference(void* ref, struct FSDriver* driver)
{
	...
	struct StorageDevice* raid = driver->driver;
	struct raid_context* context = raid->context;
	struct StorageDevice* disk = NULL;
...
void* raid1_cache_reference(int id, struct FSDriver* driver)
{
	...
	struct StorageDevice* raid = driver->driver;
	struct raid_context* context = raid->context;
	struct StorageDevice* disk = context->disks[0];
